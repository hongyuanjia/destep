...
The opt-in `hvac = "physical"` mode supports one-room
`AC_SYS_TYPE = 0` constant-volume systems and exactly two conditioned rooms
linked to a shared type-0 constant-volume or type-1 VAV terminal-reheat
system. Supported systems may use `FRESH_AIR_TYPE` 1, 5, or 6. Equipment and
zone outdoor-air parameters that are absent from the DeST model must be
provided explicitly through `hvac_options`; see `?to_eplus` for the required
fields. Other physical topologies are not projected, and source models with
an `AC_SYS_TYPE` other than 0 or 1 stop with an explicit error.
